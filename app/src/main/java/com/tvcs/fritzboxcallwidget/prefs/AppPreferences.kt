...
class AppPreferences(context: Context) {
    ...
    var fritzHost: String
        get() = prefs.getString(KEY_HOST, "fritz.box") ?: "fritz.box"
        set(v) = prefs.edit().putString(KEY_HOST, v).apply()

    var fritzPort: Int
        get() = prefs.getString(KEY_PORT, "49000")?.toIntOrNull() ?: 49000
        set(v) = prefs.edit().putString(KEY_PORT, v.toString()).apply()
    ...
    // ── Resolved color set for the current light/dark mode ─────────────────────
    fun resolvedColors(context: Context): WidgetColors {
        return if (effectiveIsDark(context)) {
            WidgetColors(
                darkHeaderBg, darkHeaderText,
                darkColHeaderBg, darkColHeaderText,
                darkWidgetBg, darkRowEven, darkRowOdd,
                darkTextPrimary, darkTextSecondary,
                darkDivider, darkError
            )
        } else {
            WidgetColors(
                lightHeaderBg, lightHeaderText,
                lightColHeaderBg, lightColHeaderText,
                lightWidgetBg, lightRowEven, lightRowOdd,
                lightTextPrimary, lightTextSecondary,
    ...
    companion object {
        ...
        val ALL_COLOR_KEYS = listOf(
            KEY_LIGHT_HEADER_BG, KEY_LIGHT_HEADER_TEXT,
            KEY_LIGHT_COL_HEADER_BG, KEY_LIGHT_COL_HEADER_TEXT,
            KEY_LIGHT_WIDGET_BG, KEY_LIGHT_ROW_EVEN, KEY_LIGHT_ROW_ODD,
            KEY_LIGHT_TEXT_PRIMARY, KEY_LIGHT_TEXT_SECONDARY,
            KEY_LIGHT_DIVIDER, KEY_LIGHT_ERROR,
            KEY_DARK_HEADER_BG, KEY_DARK_HEADER_TEXT,
            KEY_DARK_COL_HEADER_BG, KEY_DARK_COL_HEADER_TEXT,
            KEY_DARK_WIDGET_BG, KEY_DARK_ROW_EVEN, KEY_DARK_ROW_ODD,
            KEY_DARK_TEXT_PRIMARY, KEY_DARK_TEXT_SECONDARY,
